...
class FormContasPagar(FlaskForm):
    ...
    datapagar = DateField('Data a Pagar', validators=[DataRequired()], format='%Y-%m-%d')
    submit = SubmitField('Adicionar')
    submit_editar = SubmitField('Atualizar')

class FormContasReceber(FlaskForm):
    descreceber = StringField('Descrição', validators=[DataRequired(), Length(min=2, max=50)])
    valorreceber = FloatField('Valor', validators=[DataRequired()])
    datareceber = DateField('Data a Receber', validators=[DataRequired()], format='%Y-%m-%d')
    submit = SubmitField('Adicionar')
    submit_editar_r = SubmitField('Atualizar')